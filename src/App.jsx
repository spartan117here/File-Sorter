...
import { Folder, Play, FileText, Image as ImageIcon, Video, Archive, CheckCircle, AlertCircle, Settings, Search, RefreshCw } from 'lucide-react';

export default function App() {
  const [logs, setLogs] = useState([]);
  const [appState, setAppState] = useState('idle');
  const [selectedFolder, setSelectedFolder] = useState('Loading...');
  const [stats, setStats] = useState({ images: 0, docs: 0, media: 0, archives: 0 });
  const [realFiles, setRealFiles] = useState([]);

  // --- NEW: AUTO-DETECT DIRECTORY ON APP LOAD ---
  useEffect(() => {
    const initializePath = async () => {
      // 1. Check if the user saved a custom path last time
      const savedPath = localStorage.getItem('userTargetFolder');
      
      if (savedPath) {
        setSelectedFolder(savedPath);
      } else {
        // 2. If not, auto-detect the OS Desktop folder!
  ...
  // --- NEW: SAVE CUSTOM DIRECTORY ---
  const handleFolderChange = (e) => {
    const newPath = e.target.value;
    setSelectedFolder(newPath);
    localStorage.setItem('userTargetFolder', newPath); // Save it for next time!
  };

  const handleReScan = () => {
    setAppState('idle');
    setLogs([]);
    setStats({ images: 0, docs: 0, media: 0, archives: 0 });
    setRealFiles([]);
  ...
  const handleSort = async () => {
    ...
    // CALLING THE ELECTRON BACKEND TO MOVE FILES
    const response = await window.electronAPI.sortDirectory(selectedFolder, realFiles);

    if (response.success) {
      for (const file of realFiles) {
        await sleep(50); 
        let targetFolder = file.type === 'images' ? 'Images' : file.type === 'docs' ? 'Documents' : file.type === 'media' ? 'Media' : 'Archives';
